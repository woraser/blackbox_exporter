...
func TestTCPConnection(t *testing.T) {
	...
	if !ProbeTCP(testCTX, ln.Addr().String(), config.Module{TCP: config.TCPProbe{IPProtocolFallback: true}}, registry, log.NewNopLogger(), params) {
...
func TestTCPConnectionFails(t *testing.T) {
	...
	testCTX, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	if ProbeTCP(testCTX, ":0", config.Module{TCP: config.TCPProbe{}}, registry, log.NewNopLogger(), params) {
...
func TestTCPConnectionWithTLS(t *testing.T) {
	...
	if ProbeTCP(testCTX, ln.Addr().String(), module, registry, log.NewNopLogger(), params) {
	...
	if !ProbeTCP(testCTX, target, module, registry, log.NewNopLogger(), params) {
		t.Fatalf("TCP module failed, expected success.")
	}
	<-ch
...
func TestTCPConnectionProtocol(t *testing.T) {
	...
	registry := prometheus.NewRegistry()
	result := ProbeTCP(testCTX, net.JoinHostPort("localhost", port), module, registry, log.NewNopLogger(), params)
	if !result {
