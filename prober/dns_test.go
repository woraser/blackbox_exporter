...
func TestRecursiveDNSResponse(t *testing.T) {
	...
	for _, protocol := range PROTOCOLS {
		...
		for i, test := range tests {
			...
			defer cancel()
			result := ProbeDNS(testCTX, addr.String(), config.Module{Timeout: time.Second, DNS: test.Probe}, registry, log.NewNopLogger(), params)
...
func TestDNSProtocol(t *testing.T) {
	...
	for _, protocol := range PROTOCOLS {
		...
		testCTX, cancel := context.WithTimeout(context.Background(), 10*time.Second)
		defer cancel()
		result := ProbeDNS(testCTX, net.JoinHostPort("localhost", port), module, registry, log.NewNopLogger(), params)
		if !result {
